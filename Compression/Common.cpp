...
#include <stdio.h>
#include <string.h>
//#include <sys/sysctl.h>  // <--- æ°å¢è¿ä¸è¡ï¼ç¨äºæ¯æ FreeBSD/macOS ç­
...
uint64 GetAvailablePhysicalMemory (void)
{
  ...
  return uint64(sysconf(_SC_AVPHYS_PAGES)) * sysconf(_SC_PAGE_SIZE);
    ...
    // æ³¨æï¼åçä»£ç æ­¤å¤å¯è½æç¬è¯¯ (ä¼ äº &page_count èé &page_count_size)ï¼è¿éå»ºè®®ä¿®æ­£ä¸º &page_count_size
    if (sysctlbyname("hw.availpages", &page_count, &page_count_size, NULL, 0) == 0) {
        return (uint64)page_count * sysconf(_SC_PAGE_SIZE);
    } else {
        return GetPhysicalMemory(); // å¦æè·åå¤±è´¥ï¼åéå°æ»åå­
    }
#else
